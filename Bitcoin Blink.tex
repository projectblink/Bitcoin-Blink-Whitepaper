...
Delegators provide bitcoins or the staking token for accepted token id to nodes. For non-accepted tokens delegators $C$ can create their own script along with specifying node $M$.
\begin{equation*}
\mathbb{SS} = lim,M,id,R,(am_n,Pub_k,e_i) \in |\mathbb{D}|,P(am_n,Pub(M),e_i),F(am,e_i)
\end{equation*}

When deposit, each stake of delegators $|\mathbb{D}|$ is identified along with its $k$ Pubkey and $e_i$ exchange rate to deduct gains tax of staking period. During production of blocks, producers can reject their stake script transactions to freeze the collateral as a proof for validation. For accepted tokens, validators create outputs to themselves and deposit fees. For non-accepted tokens producers inflate the ratio $R$ , to withdraw staked tokens for comissions.
\begin{equation*}
P(am_n) =
\begin{cases}
R=1,P(am_n)=0 & \text{ if } Pub(M)=Pub(C)\\
R=?,P(am_n)=?  & \text{ if } Pub(M)\neq Pub(C)
\end{cases}
\end{equation*}
\begin{equation*}
Withdrawal(S) =
\begin{cases} 
[D(am_n)*R] + F(am)*[D(am_n(Pub_k))/100*\sum_1^n D(am_n)], & \text{ if } D\\
 & \text{ if } P
\end{cases}
\end{equation*}
\begin{conditions}
lim & Bitcoin or Stake Token Limit\\
id & Token id Hash\\
M & Stake Script Maintainer (Producer Node)\\
C & Stake Script Creator or Delegator \\
\end{conditions}

\section{Routing Path(Writing)}
...
\section{Size Optimization(Writing)}
\section{Improvements(Writing)}
