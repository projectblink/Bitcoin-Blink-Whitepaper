...
\title{
 \large \textbf{Bitcoin (Blink) - Peer to Peer Global Settlement Layer}\\
\vspace{6mm}
...
\begin{equation}
\varv (T_h(b_n)) = \sum_0^n \Psi
\end{equation}
\begin{equation}
x=\epsilon_{n-2}[\widetilde{x}(\sum_{b_1}^{b_n} \varv(T_h))]+ [r (T_h(\epsilon_n))(x)]
\end{equation}
\begin{equation}
\forall r(T_h(\epsilon_n)) \geq 10^{-2}
\end{equation}
\begin{equation}
r (T_h(\epsilon_n)) =  r(T_h(\epsilon_{n-1}))  +  (\frac{C_{\epsilon_{n-3}} - C_{\epsilon_{n-2}}}{10 \times C_{\epsilon_{n-2}}})
\end{equation}
where:
\begin{conditions}
x & Staking Requirement in Bitcoins\\
\widetilde{x} & Median Value\\
\varv & Volume in Market Price (\$)\\
T_h & Token ID Hash\\
\epsilon_n & Current Epoch\\
\epsilon_{n-k} & Target Epoch\\
b_n & Target Block\\
\Psi & UTXO outs $T_h(b_n)$\\
r & Interest Rate\\
...
\textit{21 million allocation with halving}
