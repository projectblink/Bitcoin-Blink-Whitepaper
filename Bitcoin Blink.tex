...
Allocation,
\begin{equation*}
\forall |\mathbb{R}''|, \Phi_{P(b)_n} = \frac{x \cdot P(w)_n}{10^4 \cdot \sum_{P(w)_n \in |\mathbb{R}''|} P(w)_n} = \mathbb{N}
\end{equation*}

\begin{conditions}
x & Number of blocks per epoch\\
j & Block size per second\\
l & Latency\\
...
To randomize the election, between a block range a seed($k$) - Merkle Chain Root from a confirmed block is found. This seed will lie between a selected range in $2^{256}$ decimal possibilities. Difficulty rate is adjusted every epoch to seed entropy. 

\begin{equation*}
Increasing Difficulty Level (d)
\end{equation*}
\begin{equation*}
Announcing the Range to Fall into
\end{equation*}
\begin{equation*}
MCR taken as seed k
\end{equation*}
