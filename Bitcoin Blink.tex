...
When deposit, each stake of delegators $(D_T)$ is identified along with its $(k)$ Pubkey and $(er_i)$ initial exchange rate to deduct gains tax during withdrawal $(W)$. Each script $(\mathbb{SS})$ will have a limit $(lim)$ in bitcoins or staking token along with $id$ for what token $(t)$ it is being staked. During production of blocks, producers $P(M)$ can reject their stake script transactions to freeze the collateral as a proof for validation. For accepted tokens, validators create outputs to themselves and deposit fees. For non-accepted tokens producers inflate the ratio $(R)$ , to withdraw staked tokens $(T)$ for comissions.
\begin{equation*}
...
W(\mathbb{SS}) =
\begin{cases} 
[D_T(am_n) \cdot R] + Fees , & \text{ if } D_T \wedge [(Pub(M)= h1) \vee (Pub(M)\neq h1) \in D_T] \\
F(am)_t \cdot C & \text{ if } P \wedge Pub(M)= h1 \in D_T\\
Remaining of D_t from R & \text{ if } P \wedge Pub(M)\neq h1 \in D_T\\
\end{cases}
\end{equation*}
