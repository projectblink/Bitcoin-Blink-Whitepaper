...
\begin{equation*}
\forall \mathbb{R}\{P_1(b_1),\dots P_n(b_n)\} \begin{cases}
\mathbb{R} \backslash \{\{P_n(b_n)\}\} \wedge \in \mathbb{R}', & \text{if } b_n < \bar{x}(\mathbb{R}) \\
P_n(b_n) \in \mathbb{R}  & \text{,otherwise}
\end{cases}
\end{equation*}
After Voting,
\begin{equation*}
k = \sum_{P'_n(b'_n)}^{P'_m(b'_m)} \text{ only,} \in \mathbb{R}
\end{equation*}
\begin{equation*}
\forall \mathbb{R'} \backslash \{\{P_n(b_n)\}\}, \text{ if } P_n(b_n) \in \mathbb{R}' \text{ where, } k > \sum_{b_1}^{b_n} \mathbb{R} \cdot (51 \cdot 10^{-2})
\end{equation*}
\begin{equation*}
P_n(b_n) \in \mathbb{R}',  \text{ otherwise}
\end{equation*}
...
P_n & PubKey\\
b_n & Bandwidth in bps \\
P'_n(b'_n) & Voted Pubkey's bps\\
i & Joining requirement in bps 
\end{conditions}

\subsection{Producer Allocation}

%Blocks are allocated for a whole epoch during the election commneced from a randomized seed. Every node constructs or validates the same predictable election result. Firstly, total number of blocks allocated per producer is calculated by weights including bandwidth in bps and honesty weights.

%Total blocks allocation per individual (only int value) = total blocks per epoch * (sum of individual bandwidth plus all weights / sum of all nodes weight (bandwidths plus node weights) of current bandwidth proofs  *100) / 100

%When Total allocated blocks is lesser than total blocks per epoch, the rest of blocks are assigned to the first of blocks
