...
\section{Stake Proof(Writing)}

Delegators provide bitcoins or the staking token for accepted token id ($\iff Pub(M)=Pub(C)$) to nodes. For non-accepted tokens delegators $C$ can create their own script along with specifying node $M$.
\begin{equation*}
\mathbb{SS} = lim,M,id
\end{equation*}
Deposit,
...
\end{equation*}
\begin{equation*}
User Deposit with Pubkey
\end{equation*}
Accepted tokens fees,
\begin{equation*}
Deposit Fees excluding comission
\end{equation*}
\begin{equation*}
Direct Outputs to Node \& DAO 
\end{equation*}
Non Accepted tokens,
\begin{equation*}
Inflate Ratio
...
Withdrawal,
\begin{equation*}
Any Withdraw to pubkey
\end{equation*}
\begin{conditions}lim & Bitcoin or Stake Token Limit\\
id & Token id Hash\\
M & Stake Script Maintainer (Producer Node)\\
C & Stake Script Creator or Delegator \\
\end{conditions}
During production of blocks, producers can reject their stake script transactions to freeze the collateral as a proof for validationZxcte.
